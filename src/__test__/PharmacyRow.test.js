import PharmacyRow from '../components/PharmacyRow';
import { BrowserRouter as Router } from 'react-router-dom';
import { render, screen, fireEvent } from "@testing-library/react";
const pharmacyData = {
    date: "1/12/2021",
    pharmacy: "Guyon",
    name: "Rowland",
    phone: "481-638-2241",
    address: "70182 Rutledge Junction",
    status: "processing"
}
test('comment box is opened when open button is clicked', () => {
    
    render(<Router><PharmacyRow pharmacy={pharmacyData} /></Router>);

    //initially tooltip is not visible

    const initialCommentTooltip = screen.queryByTestId('comment-tooltip');
    expect(initialCommentTooltip).not.toBeInTheDocument();

    const openButton = screen.getByRole('button', { name: /open/i });
    fireEvent.click(openButton);
    const commentTooltip = screen.getByTestId('comment-tooltip');
    expect(commentTooltip).toBeInTheDocument();

})

test('when status is processing request registration button must be disabled ',()=>{
    render(<Router><PharmacyRow pharmacy={pharmacyData} /></Router>);
    const registrationButton= screen.getByRole('button', { name: /request sent/i });
    expect(registrationButton).toBeDisabled();
})